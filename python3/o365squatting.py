...
class DomainSquatting():
  ...
  def __homoglyph(self):
    glyphs = {
    'b': ['d', 'lb'],'c': ['e'],'d': ['b', 'cl', 'dl'],'e': ['c'],'g': ['q'],'h': ['lh'],'i': ['1', 'l'],'k': ['lk', 'ik', 'lc'],'l': ['1', 'i'],'m': ['n', 'nn', 'rn', 'rr'],'n': ['m', 'r'],'o': ['0'],'q': ['g'],'u': ['v'],'v': ['u'],'w': ['vv', 'uu'],'z': ['s']
    }
    result_1pass = set()
...
def output(domain,list,type):
  ...
  if type=="csv":
    ...
    for i in list:
      outputline.writerow([i,'Detected!'])
    outputcsv.close()
...
def single(domain, debug):
    if debug:
        print('[DEBUG] Generating list of all domains impersonating %s' % domain)
